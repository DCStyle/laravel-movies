...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg p-4 sm:p-6 mb-8">
            ...
            <form action="{{ route('management.footer.column.store') }}" method="POST" class="mb-6">
                @csrf
                ...
                @foreach ($columns as $column)
                    <div class="column-item border p-3 sm:p-4 rounded-lg draggable-item" data-id="{{ $column->id }}">
                        ...
                        <div class="items-list bg-gray-50 p-3 sm:p-4 rounded-lg">
                            <!-- Add Link Form -->
                            <form action="{{ route('management.footer.column.item.store', $column) }}" method="POST"
	    ...
	    document.addEventListener('DOMContentLoaded', function () {
		    ...
		    Sortable.create(columnList, {
			    ...
			    onEnd: function(evt) {
				    ...
				    $.post('/management/footer/column/update-order', {
					    order: order,
					    _token: '{{ csrf_token() }}'
				    });
			    }
		    ...
		    document.querySelectorAll('.items-list').forEach(function (list) {
			    Sortable.create(list.querySelector('.space-y-2'), { // Target the container of items
				    ...
				    onEnd: function(evt) {
					    ...
					    $.post('/management/footer/item/update-order', {
						    order: order,
						    _token: '{{ csrf_token() }}'
					    });

					    if (evt.from !== evt.to) {
						    $.post('/management/footer/item/update-parent', {
							    item_id: itemId,
	    ...
	    document.addEventListener('DOMContentLoaded', function () {
		    ...
		    $('.edit-btn').on('click', function () {
			    ...
			    if (inputs.first().hasClass('hidden')) {
				    ...
				    $.post('/management/footer/update', data)
					    .done(() => {
